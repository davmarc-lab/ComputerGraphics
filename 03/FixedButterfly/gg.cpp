...
void drawScene(void)
{
	...
	for (k = 0; k < Scena.size(); k++) {
		if (k == 0) {
			...
			//Scena[k].Model = translate(Scena[k].Model, vec3(800.0 + dx, 400.0 + dy, 0.0)); //Scommentare per muovere, ruotare e scalare
			Scena[k].Model = translate(Scena[k].Model, vec3(px, py, 0.0));
...
void updateAngoloCerchio(int value) {
	angoloCerchio += 1.0;
	glutTimerFunc(250, updateAngoloCerchio, 0);
	glutPostRedisplay();
}
...
void myKeyboard(unsigned char key, int x, int y) {
	...
	case 's':
		...
		break;
	case 'q':
		angoloRotazione = angoloRotazione + 5.0;
		break;
	case 'e':
		angoloRotazione = angoloRotazione - 5.0;
		break;
