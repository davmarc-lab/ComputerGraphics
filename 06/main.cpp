...
void INIT_SHADER(void)
{
    ...
    char* fragmentShader = (char*)"fragShader_onde.glsl";
...
void INIT_VAO(void)
{
    ...
    for (int i = 0; i < nEnemies; i++)
    {
        ...
        tmp.nTriangles = 180;
        tmp.s = 0.2;
        costruisci_farfalla(0, 0, tmp.s, tmp.s, &tmp);
        crea_VAO_Vector(&tmp);
        tmp.render = GL_TRIANGLE_FAN;
        tmp.alive = true;
    ...
    locCol3 = glGetUniformLocation(programId, "skyColor"); ; // Colore del cielo
...
void drawScene(void)
{
    ...
    Scena[2].Model = translate(Scena[2].Model, vec3(600.0 + dx_t, 60.0 + dy_t, 0.0));
    Scena[2].Model = scale(Scena[2].Model, vec3(0.9, 0.9, 1));
    ...
    for (int i = 0; i < nEnemies; i++)
    {
        enemies[i].Model = mat4(1.0f);
        vec2 position = vec2(i * (width / nEnemies), 500);
        enemies[i].Model = translate(enemies[i].Model, vec3(position.x + 50 * cos(dx_f), position.y + 50 * sin(dx_f), 0));
        enemies[i].Model = scale(enemies[i].Model, vec3(80.5, 80.5, 1.0));

        enemies[i].corner_b = enemies[i].corner_b_obj;
        enemies[i].corner_t = enemies[i].corner_t_obj;
        //printf("Farfalla \n");
        enemies[i].corner_b = enemies[i].Model * enemies[i].corner_b;
        //std::cout << glm::to_string(Scena[0].corner_b) << std::endl;
        enemies[i].corner_t = enemies[i].Model * enemies[i].corner_t;


        Coll = checkCollision(enemies[i], Scena[3]);
        if (Coll == true)
            enemies[i].alive = false;

        if (enemies[i].alive == true) 
        {
            glUniform1i(locSceltafs, enemies[i].sceltaFs);
            glUniformMatrix4fv(MatModel, 1, GL_FALSE, value_ptr(enemies[i].Model));
            glBindVertexArray(enemies[i].VAO);
            glDrawArrays(GL_TRIANGLE_FAN, 0, enemies[i].nv - 6);
            if (drawBB == true)
                //		//Disegno Bounding Box
                glDrawArrays(GL_LINE_STRIP, enemies[i].nv - 6, 6);
            glBindVertexArray(0);
        } else {
            int countAlive = 0;
