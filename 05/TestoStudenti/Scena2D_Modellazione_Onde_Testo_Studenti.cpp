...
void drawScene(void)
{
	...
	for (k = 0; k < Scena.size(); k++)
	{
		 if (k==0)
		 {
			...
			glUniform1f(loctime, time / 1000.0);
			glBindVertexArray(Scena[k].VAO);
			glDrawArrays(Scena[k].render, 0, Scena[k].nv);
			glBindVertexArray(0);
	}
	Curva.Model = mat4(1.0);
	Curva.Model = translate(Curva.Model, vec3(700.0, 300.0, 0.0));
...
int main(int argc, char* argv[])
{
	...
	glutTimerFunc(250,update_Barca, 0); //gestione evento oziosit : viene richiamata la funzione updateScale ogni 250 millisecondi che aggiorna i parametri e  forza il ridisegno
	...
	INIT_VAO();
	//Init VAO_Text per la gestione del testo
