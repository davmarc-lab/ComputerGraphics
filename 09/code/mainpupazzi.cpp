    ...
    for (int i = -3; i < -3 ; i++)
    {
        ...
        for (int j = -3; j < -3; j++)
        {
            ...
            for (int k = 0; k < Snowman.size(); k++)
            {
                ...
                Snowman[k].ancora_world = Snowman[k].ancora_obj;
                Snowman[k].ancora_world = Snowman[k].Model * Snowman[k].ancora_world;
                //Passo al Vertex Shader il puntatore alla matrice Model dell'oggetto k-esimo della Scena, che sarà associata alla variabile Uniform mat4 Projection
                //all'interno del Vertex shader. Uso l'identificatio MatModel
                mat4 model = translate(mat4(1.0f), vec3(traslax, traslay, 0)) * Snowman[k].Model;

                glUniform1i(locSceltaVs, Snowman[k].sceltaVS);
