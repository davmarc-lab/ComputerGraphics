...
void INIT_VAO(void)
{
	...
	crea_piano_suddiviso(&oggetto,vec4(0.5,0.5,0.5,1.0));
	...
	//Crea Bandiera
	crea_cilindro(&oggetto, vec4(1.0, 0.0,0.0, 1.0));
	crea_VAO_Vector(&oggetto);
	oggetto.Model = mat4(1.0);
	...
	oggetto.normali.clear();
	//
	crea_cono(&oggetto, vec4(1.0, 1.0, 0.0, 1.0));
	crea_VAO_Vector(&oggetto);
	oggetto.Model = mat4(1.0);
	oggetto.Model = translate(oggetto.Model, vec3(-2.0, -0., 3.0));
	...
	//FALDA
	crea_toro(&oggetto, vec4(1.0, 0.0, 1.0, 1.0));
	crea_VAO_Vector(&oggetto);
	oggetto.Model = mat4(1.0);
	oggetto.Model = translate(oggetto.Model, vec3(0.0, 2.3, 0.0));
